	...
	public class Mixer
	{
		...
		private static bool SetVolume(int mixerID, uint dst_type, uint src_type, int val)
		{
			...
			if(success == true)
			{
				...
				retval = mixerSetControlDetails(mixer, ref mxcd, MIXER_SETCONTROLDETAILSF_VALUE); 
				if(retval == MMSYSERR_NOERROR) 
				{
		...
		public static int GetMux(int mixerID)
		{
			...
			if(!GetControlByType(mixer, MIXERLINE_COMPONENTTYPE_DST_WAVEIN, 0, MIXERCONTROL_CONTROLTYPE_MUX, out ctrl))
			{
				retval = -1;
				goto end;
			}
		...
		public static bool InitUnsupportedCard(int mixerID)
		{
            ...
            SetLineInMute(mixerID, true);
			SetMicMute(mixerID, true);
		...
		public static bool InitSantaCruz(int mixerID)
		{
			...
			// Set Playback Main Volume to max
			MIXERCONTROL ctrl = new MIXERCONTROL();
			bool success = GetControlByID(mixer, 0, -1, MIXERCONTROL_CONTROLTYPE_VOLUME, out ctrl);
			if(!success)
				goto failed;
			if(!SetUnsigned(mixer, ctrl, ctrl.lMaximum))
				goto failed;

			// Set Playback Main Mute to off
		...
		public static bool InitAudigy2(int mixerID)
		{
			...
			// turn Main Mute Off
			MIXERCONTROL ctrl = new MIXERCONTROL();
			bool success = GetControlByID(mixer, 0, -1, MIXERCONTROL_CONTROLTYPE_MUTE, out ctrl);
			if(!success)
				goto failed;
			if(!SetBool(mixer, ctrl, false))
				goto failed;

			// turn Playback Wave/MP3 Mute off
			success = GetControlByID(mixer, 0, 0, MIXERCONTROL_CONTROLTYPE_MUTE, out ctrl);
			if(!success)
				goto failed;
			if(!SetBool(mixer, ctrl, false))
			...
			// set Playback Wave/MP3 Volume to max
			success = GetControlByID(mixer, 0, 0, MIXERCONTROL_CONTROLTYPE_VOLUME, out ctrl);
			if(!success)
				goto failed;
			if(!SetUnsigned(mixer, ctrl, ctrl.lMaximum))
			...
			ctrl = new MIXERCONTROL();
			success = GetControlByID(mixer, 1, 3, MIXERCONTROL_CONTROLTYPE_ONOFF, out ctrl);
			if(!success)
				goto failed;
			if(!SetBool(mixer, ctrl, true))
				goto failed;
		...
		public static bool InitAudigy2ZS(int mixerID)
		{
			int mixer;
			int retval = -1;
			retval = mixerOpen(out mixer, mixerID, 0, 0, 0);
